...
- **Visual editor**: `colloquium edit deck.md` (or just `colloquium edit` for a file picker) opens a NiceGUI editor: slide list, the real build in a preview with drag/resize handles for placed elements, and an inspector for layout directives, column/row cell markdown, placed text, image replace/crop, z-index and rotation. Double-click any text (title, column, block, placed text) to edit its source in place; a hand-written `<div style="top:..;left:..">` callout keeps its position and becomes a `place` block the first time you drag it. Every change is written straight back to the `.md` with string-level edits, so untouched slides never show up in a diff; external edits to the file are picked up live. Install with `uv pip install -e ".[editor]"` (adds `nicegui` and `pillow`).
- **Editor, Slides/PowerPoint parity**: real slide thumbnails with drag-and-drop reordering; shift-click multi-select with group drag; Arrange (bring to front/back, forward/backward, Ctrl+↑/↓), align left/centre/right/top/middle/bottom and distribute; duplicate (Ctrl+D), copy/paste between slides (Ctrl+C/V); one-click conversion of inline images to placed (movable, croppable) images; shapes (`shape: rect | rounded | ellipse | line | arrow` with fill, stroke and centred label).
- **Editor, text and image tools**: a formatting toolbar above the preview for placed text and positioned HTML (bold/italic/code, size, colour, background, border, padding, alignment; Ctrl+B/I also work inside the in-place editor); inline images (`![..]()` and `<img>`) get handles too: corner/side drags resize them in place by writing a width (aspect kept), dragging the image lifts it out of the flow into a placed image at the drop point; Slides-style crop directly on the canvas (drag the orange handles to cut, drag inside to move the image, Enter applies), reset size to natural pixels, reset crop; Ctrl+Shift+V pastes without offset. Everything is undoable (Ctrl+Z / Ctrl+Y or Ctrl+Shift+Z).
- **Editor, everything is an object**: every markdown block in a column/row (paragraph, list, heading, chart) gets selection handles too — dragging or resizing it lifts it out of the flow into a `place` block at the drop position; cells are resizable by dragging the blue divider between them (rewrites the `columns:`/`rows:`/`row-columns:` fractions) and stylable per cell (the toolbar's alignment/colour/size writes a `<!-- cell-style: ... -->` comment that the build applies to that cell); Ctrl+G groups the selection (writes a `group:` key on the place blocks, converting positioned HTML first) so it selects, moves, aligns and resizes as one — Alt+click picks a single member, Ctrl+Shift+G ungroups.
- **Starter template**: `colloquium new talks/my-talk` (or the "New deck from template" button when `colloquium edit` starts without a file) creates a clean four-slide deck from `colloquium/templates/starter/`: a theme slide with a placed logo and footer text, a title slide, a two-column slide with draggable blocks and an inline image, and a free-placement slide with a cropped image and a grouped shape/arrow/text. Each slide's speaker notes say what to try.
- **Theme slide**: a slide marked `<!-- master: true -->` (the editor's "Theme slide" button adds one at the top) is never part of the presentation; every `place` element on it repeats on all other slides, behind their content (give an element `z:` to put it in front), and a slide opts out with `<!-- master: off -->`. In the editor the theme slide is edited like any other slide (drag a logo into place once, it moves everywhere), stamped elements are shown locked on normal slides and double-click jumps to the theme slide, and its inspector is a Theme panel: colour pickers and font stacks for the `--colloquium-*` variables, slide background image and padding, and the raw `custom_css`, all written string-level into the frontmatter.
