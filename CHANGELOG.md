...
- Editor v2 internals: blocks are enumerated with markdown-it's tokenizer (raw HTML split per element) so browser and source always agree; positioned HTML and inline images are ordinary blocks that convert to `place` when dragged (in-place image resize kept); one crop UI and one formatting surface (toolbar); `app.py` split into state/page/actions/inspectors/toolbar/events with NiceGUI `User` tests; per-tab sessions share one deck state (other tabs refresh within a second); single-slide rebuild for slide-local edits
- `colloquium new <folder>` and a "New deck from template" button in the editor's file picker create a neutral starter deck (theme slide, title, columns with inline image, free placement with crop and group) from `colloquium/templates/starter/`
